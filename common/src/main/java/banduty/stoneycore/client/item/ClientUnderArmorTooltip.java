...
public class ClientUnderArmorTooltip implements ClientTooltipComponent {
    ...
    public int getWidth(Font font) {
        return Math.min(ArmorAttachmentSlotDefinitionsStorage.getSlotsForArmorType(armorType).size(), COLUMNS) * SLOT_SIZE;
    }

    private int gridSizeY() {
        return Mth.positiveCeilDiv(ArmorAttachmentSlotDefinitionsStorage.getSlotsForArmorType(armorType).size(), COLUMNS);
    }

    @Override
    public void renderImage(Font font, int x, int y, GuiGraphics guiGraphics) {
        for (int i = 0; i < ArmorAttachmentSlotDefinitionsStorage.getSlotsForArmorType(armorType).size(); i++) {
            ArmorAttachmentSlotDefinitionData slotDef = ArmorAttachmentSlotDefinitionsStorage.getSlotsForArmorType(armorType).get(i);
