...
public record StackIngredient(
        ItemStack stack,
        Optional<TagKey<Item>> tag,
        FinishedRequirement finishedRequirement
) {
    ...
    public enum FinishedRequirement {
        ...
        public static final StreamCodec<RegistryFriendlyByteBuf, FinishedRequirement> STREAM_CODEC =
                ByteBufCodecs.idMapper(
                        i -> FinishedRequirement.values()[i],
                        FinishedRequirement::ordinal
                ).cast();
    }
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, TagKey<Item>> TAG_STREAM_CODEC =
            ResourceLocation.STREAM_CODEC.<RegistryFriendlyByteBuf>cast()
                    .map(
                            location -> TagKey.create(Registries.ITEM, location),
                            TagKey::location
                    );

    public static final MapCodec<StackIngredient> CODEC =
            RecordCodecBuilder.mapCodec(inst -> inst.group(
                    ItemStack.OPTIONAL_CODEC
                            .fieldOf("stack")
                            .forGetter(StackIngredient::stack),

                    TagKey.codec(Registries.ITEM)
                            .optionalFieldOf("tag")
                            .forGetter(StackIngredient::tag),

                    FinishedRequirement.CODEC
                            .optionalFieldOf(
                                    "finished_requirement",
                                    FinishedRequirement.ANY
                            )
                            .forGetter(StackIngredient::finishedRequirement)
    ...
    public boolean test(ItemStack input) {
        ...
        /*
         * IGNITED = true  -> item is NOT finished
         * IGNITED = false/absent -> item IS finished
         */
        boolean isIgnited = input.getOrDefault(
                SCDataComponents.IGNITED.get(),
                false
        );

        boolean isFinished = !isIgnited;
    ...
    private boolean areItemsEqualIgnoringDataComponents(
            ItemStack stack1,
            ItemStack stack2
    ) {
        ...
        /*
         * These components represent the item's temporary
         * ignition state and must not affect recipe matching.
         */
        patched1.remove(SCDataComponents.IGNITE_TIME.get());
        patched2.remove(SCDataComponents.IGNITE_TIME.get());

        patched1.remove(SCDataComponents.IGNITED.get());
        patched2.remove(SCDataComponents.IGNITED.get());
