...
public class SCUnderArmourRenderer implements IClientItemExtensions {
    ...
    public @NotNull HumanoidModel<?> getGenericArmorModel(LivingEntity entityLiving, ItemStack itemStack, EquipmentSlot armorSlot, HumanoidModel<?> _default) {
        if (HELMET_MODEL == null) {
            EntityModelSet models = Minecraft.getInstance().getEntityModels();
            HELMET_MODEL = new UnderArmourHelmetModel(models.bakeLayer(UnderArmourHelmetModel.LAYER_LOCATION));
            CHEST_MODEL = new UnderArmourChestplateModel(models.bakeLayer(UnderArmourChestplateModel.LAYER_LOCATION));
            LEGGINGS_MODEL = new UnderArmourLeggingsModel(models.bakeLayer(UnderArmourLeggingsModel.LAYER_LOCATION));
            BOOTS_MODEL = new UnderArmourBootsModel(models.bakeLayer(UnderArmourBootsModel.LAYER_LOCATION));
        }
