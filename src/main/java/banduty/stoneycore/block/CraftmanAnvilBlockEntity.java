...
public class CraftmanAnvilBlockEntity extends BlockEntity implements ImplementedInventory {
    ...
    public CraftmanAnvilBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CRAFTMAN_ANVIL_BLOCK_ENTITY, pos, state);
    ...
    public Optional<AnvilRecipe> getRecipe() {
        ...
        return level.getRecipeManager().getRecipeFor(ModRecipes.ANVIL_RECIPE_TYPE, this, level);
    }
    ...
    public void checkAndSpawnRecipeParticles() {
        ...
        if (!hasItems) {
            // No items in the anvil
            if (lastRecipeValid) {
                // Changed from valid to empty
        ...
        if (currentRecipeValid && !lastRecipeValid) {
            // Changed from invalid/no recipe to valid recipe
            spawnParticles(ParticleTypes.HAPPY_VILLAGER, 10); // Green particles
            lastRecipeValid = true;
        } else if (!currentRecipeValid && lastRecipeValid) {
            // Changed from valid recipe to invalid
            spawnParticles(new DustParticleOptions(new Vector3f(1.0f, 0.0f, 0.0f), 1.0f), 10); // Red particles
            lastRecipeValid = false;
        }
    }
    ...
    private void completeCrafting(AnvilRecipe recipe) {
        ...
        for (ItemStack remainder : remainders) {
            if (!remainder.isEmpty()) {
                ...
                for (int j = 1; j < 7; j++) {
                    if (!items.get(j).isEmpty() && ItemStack.isSameItem(items.get(j), remainder)) {
    ...
    private Pig createTempEntity(Level level) {
